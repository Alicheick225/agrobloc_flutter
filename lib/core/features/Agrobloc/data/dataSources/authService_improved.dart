...
class AuthService {
  ...
  bool _isHtmlResponse(String responseBody) {
    return responseBody.trim().toLowerCase().startsWith('<!doctype html') ||
           responseBody.trim().toLowerCase().startsWith('<html');
  }

  /// Vérifie si l'endpoint de refresh est indisponible
  Future<bool> _isRefreshEndpointUnavailable() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final unavailable = prefs.getBool('refresh_endpoint_unavailable') ?? false;
      final lastCheck = prefs.getInt('refresh_endpoint_last_check') ?? 0;
      final now = DateTime.now().millisecondsSinceEpoch ~/ 1000;

      // Reset availability flag after 5 minutes
      if (unavailable && (now - lastCheck) > 300) {
        await _setRefreshEndpointAvailable(true);
        return false;
      }

      return unavailable;
    } catch (e) {
      print('❌ AuthService._isRefreshEndpointUnavailable() - Erreur: $e');
      return false;
    }
  }

  /// Définit la disponibilité de l'endpoint de refresh
  Future<void> _setRefreshEndpointAvailable(bool available) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('refresh_endpoint_unavailable', !available);
      await prefs.setInt('refresh_endpoint_last_check', DateTime.now().millisecondsSinceEpoch ~/ 1000);
      print('🔍 AuthService._setRefreshEndpointAvailable() - Endpoint de refresh marqué comme ${available ? 'disponible' : 'indisponible'}');
    } catch (e) {
      print('❌ AuthService._setRefreshEndpointAvailable() - Erreur: $e');
    }
  }
  ...
  Future<Map<String, String>> refreshToken(String refreshToken) async {
    ...
    // Check if refresh endpoint is known to be unavailable
    if (await _isRefreshEndpointUnavailable()) {
      print('⚠️ AuthService.refreshToken() - Endpoint de refresh connu comme indisponible, abandon');
      throw Exception("Endpoint de refresh non disponible");
    }
    ...
    while (refreshAttempt < maxRefreshRetries) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          // Reset endpoint availability flag on success
          await _setRefreshEndpointAvailable(true);
          ...
          final isHtmlResponse = _isHtmlResponse(response.body);
          if (isHtmlResponse) {
            print('⚠️ AuthService.refreshToken() - Réponse HTML détectée au lieu de JSON');
          ...
          if (response.statusCode == 401) {
            ...
            // Mark refresh endpoint as unavailable
            await _setRefreshEndpointAvailable(false);
