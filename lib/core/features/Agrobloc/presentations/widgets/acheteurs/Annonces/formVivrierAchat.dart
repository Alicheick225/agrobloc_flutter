...
class FormVivrierAchat extends StatefulWidget {
  const FormVivrierAchat({super.key});

  @override
  State<FormVivrierAchat> createState() => _FormVivrierAchatState();
}

class _FormVivrierAchatState extends State<FormVivrierAchat> {
  ...
  Future<void> _loadCultures() async {
    ...
    try {
      ...
      final cultures = allCultures.where((c) => c.type?.toLowerCase() == 'vivrière').toList();
      setState(() {
        _cultures = cultures.map((c) => {'id': c.id, 'libelle': c.libelle}).toList();
      });
    } catch (e) {
